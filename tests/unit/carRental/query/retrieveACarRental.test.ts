...
describe.each([
    ...
    'Scenario: A simple car rental retrieval ' +
        ...
        'When I retrieve car rental with ID $carRental.id ',
    (testCase) => {
        ...
        beforeEach(async () => {
            ...
            const carRentalReadRepository: InMemoryCarRentalReadRepository = container.resolve(
                'CarRentalReadRepositoryInterface',
            );
            uc = new RetrieveACarRental({
                carRentalReadRepository,
            });
            query = { id: testCase.carRental.id };
            expectedCarRental = {
                id: testCase.carRental.id,
                customerId: testCase.carRental.customerId,
                pickupDateTime: dateParser.parse(testCase.carRental.pickupDateTime),
